...
double f2(const double *  x){
  nc++;
  return TMath::Exp(-argb(x))*((2*n1func(x)*n2func(x))+n1func(x))*pol(x)*
...
int yieldv2()
{
  ...
  while (1) {
	...
	for (Int_t j=0; j<70; j++) {
	  ...
	  std::cout << "integral yield: " << val << " " << omegaq << std::endl;
	  status += std::fabs(val-RESULT) > ERRORLIMIT;
	  	  
	  /////v2 integral///////////////
	  ROOT::Math::Functor wfv2(&f2v2,2);
	  double av2[2] = {0,0};
	  double bv2[2] = {omegaq,qint};
	  ROOT::Math::GSLMCIntegrator * igv2 = new ROOT::Math::GSLMCIntegrator( ROOT::Math::IntegrationMultiDim::kVEGAS, 1.E-6, 1.E-4, 100);
	  igv2->SetFunction(wfv2);
	  double valv2 = igv2->Integral(av2,bv2);
	  //   std::cout << "integral result: " << val << " " << ig2->ChiSqr() << std::endl;
	  std::cout << "integral v1: " << valv2 << " " << omegaq << std::endl;
	  status += std::fabs(valv2-RESULT) > ERRORLIMIT;
