...
void  testIntegPerf(double x1, double x2, int n = 100000){
  ...
  //FILE *fp = fopen("datos20-40.t0.02.15.txt","r");
  //Float_t t,eBxm,eBy,eBym,eBzm, eBmpi,eB,V;
  FILE *fp = fopen("0.40.Cu.2.txt","r");
  ...
  gsT->Fill();
  char name[60];
  sprintf(name,"pruebaCu.root");
  cout << " Writtiing file ->" << name << endl;
  TFile fOut(name, "recreate");
  fOut.cd();
  gsT->Write();
}
void integralCu(double a = 0.5, double b = 3.0)
{
  testIntegPerf(a, b);
}
