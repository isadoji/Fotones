...
void  testIntegPerf(double x1, double x2, int n = 100000){
  ...
  FILE *fp = fopen("datos0.03.txt","r");
  Float_t b,eB,eBxm,eBy,eBym,V,N;
  Int_t ncols;
  Int_t nlines = 0;


    while (1) {
    ncols = fscanf(fp,"%f %f %f %f %f %f %f",&b, &eB, &eBxm, &eBy, &eBym, &V, &N);
    if (ncols < 0) break;    
    printf("b=%8f, eB=%8f, V=%8f\n",b,eB,V);
    B = eB;

    for (Int_t k=0; k<2; k++) {
      double eq = eqa[k];
      double Vol= V*alphaem*alphas*alphas*eq*eq*tau*TMath::Power(2,-1)*TMath::Power(2*TMath::Pi(),-6);
  ...
  gsT->Fill();
  char name[60];
  sprintf(name,"integral0.03fm.root");
  cout << " Writtiing file ->" << name << endl;
  TFile fOut(name, "recreate");
  fOut.cd();
  gsT->Write();
  }
void integralv2(double a = 0.1, double b = 3.5)
{
  testIntegPerf(a, b);
}
